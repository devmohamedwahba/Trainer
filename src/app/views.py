...
from .models import PackageGroup, GreatTransformation, Setting, Transformation, FoodPackages, CustomEmails
...
class QuestionnaireView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        """
        # email = CustomEmails(trainer_name=name, email=message)
        # email.save()
...
"""
        for item_key, item_value in data.items():
            if item_key == "csrfmiddlewaretoken" or item_key == "food_name":
                continue
            package = f'Choose from {item_key} options: {item_value}\n'
            message += package
        # print(message)
        ...
        food_packages = FoodPackages.objects.all()
        messages.success(request, 'Thanks for your Information We Will respond you back as soon as possible. ')

        return render(request, 'food.html',  {"food_packages": food_packages})


class FoodImagesView(View):
    template_name = 'food_images.html'
...
"""
        ...
        return render(request, 'food_images.html',  {"food_packages": food_packages})
